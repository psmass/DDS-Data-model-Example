...
unsigned int process_data(Sensor::HumiditySensorDataReader *typed_reader)
{
    Sensor::HumiditySensorSeq data_seq; // Sequence of received data
    ...
    for (int i = 0; i < data_seq.length(); ++i)
    {
        ...
        {
            // Print data
            Sensor::HumiditySensorTypeSupport::print_data(&data_seq[i]);
            samples_read++;
        }
        else
...
int run_subscriber_application(unsigned int domain_id, unsigned int sample_count)
{
    ...
    // This DataReader reads data on "Example Sensor_HumiditySensor" Topic
    ...
    Sensor::HumiditySensorDataReader *typed_reader =
        Sensor::HumiditySensorDataReader::narrow(untyped_reader);
...
int main(int argc, char *argv[])
{
    ...
    std::cout << "*** Starting Sensor Subscriber on Domain " << arguments.domain_id << ". CTRL-C to exit. ***" << std::endl;
