...
int run_publisher_application(unsigned int domain_id, unsigned int sample_count)
{
    ...
    const char *type_name = Sensor::GasSensorTypeSupport::get_type_name();
    ReturnCode_t retcode =
    Sensor::GasSensorTypeSupport::register_type(participant, type_name);
    ...
    // This DataWriter writes data on "Example Sensor_GasSensor" Topic
    ...
    Sensor::GasSensorDataWriter *typed_writer =
    Sensor::GasSensorDataWriter::narrow(untyped_writer);
    ...
    Sensor::GasSensor *data = Sensor::GasSensorTypeSupport::create_data();
    if (data == NULL) {
        return shutdown_participant(
            participant,
            "Sensor::GasSensorTypeSupport::create_data error",
            EXIT_FAILURE);
    }
    ...
    ++samples_written) {
        ...
        std::cout << "Writing Sensor::GasSensor, count " << samples_written 
        << std::endl;
    ...
    // Delete previously allocated Sensor::GasSensor, including all contained elements
    retcode = Sensor::GasSensorTypeSupport::delete_data(data);
    if (retcode != RETCODE_OK) {
        std::cerr << "Sensor::GasSensorTypeSupport::delete_data error " << retcode
        << std::endl;
    }
