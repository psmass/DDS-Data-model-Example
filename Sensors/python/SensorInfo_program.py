...
import argparse
import sys
from SensorInfo_publisher import Sensor_GasSensorPublisher
from SensorInfo_subscriber import Sensor_GasSensorSubscriber
...
def main():
    ...
    if(args.pub):
        print(f"Running Sensor_GasSensorPublisher on domain {args.domain}")
        Sensor_GasSensorPublisher.run_publisher(
            domain_id=args.domain,
            sample_count=args.sample_count)
    else:
        print(f"Running Sensor_GasSensorSubscriber on domain {args.domain}")
        Sensor_GasSensorSubscriber.run_subscriber(
            domain_id=args.domain,
            sample_count=args.sample_count)
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Runs Sensor_GasSensorPublisher or Sensor_GasSensorSubscriber')
