...
int run_publisher_application(unsigned int domain_id, unsigned int sample_count)
{
    ...
    const char *type_name = ExCmdRsp::DeviceStateTypeSupport::get_type_name();
    ReturnCode_t retcode =
    ExCmdRsp::DeviceStateTypeSupport::register_type(participant, type_name);
    ...
    ExCmdRsp::DeviceStateDataWriter *typed_writer =
    ExCmdRsp::DeviceStateDataWriter::narrow(untyped_writer);
    ...
    ExCmdRsp::DeviceState *data = ExCmdRsp::DeviceStateTypeSupport::create_data();
    if (data == NULL) {
        return shutdown_participant(
            participant,
            "ExCmdRsp::DeviceStateTypeSupport::create_data error",
            EXIT_FAILURE);
    }
    ...
    ++samples_written) {
        ...
        std::cout << "Writing ExCmdRsp::DeviceState, count " << samples_written 
        << std::endl;
    ...
    // Delete previously allocated ExCmdRsp::DeviceState, including all contained elements
    retcode = ExCmdRsp::DeviceStateTypeSupport::delete_data(data);
    if (retcode != RETCODE_OK) {
        std::cerr << "ExCmdRsp::DeviceStateTypeSupport::delete_data error " << retcode
        << std::endl;
    }
