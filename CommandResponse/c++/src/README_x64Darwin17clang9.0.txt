...
data type DeviceState from CommandResp.idl.
This example builds two applications, named CommandResp_publisher and
CommandResp_subscriber.
...
> make -f makefile_CommandResp_x64Darwin17clang9.0
This command will build a release executable.
 
To build a debug version instead:
> make -f makefile_CommandResp_x64Darwin17clang9.0 DEBUG=1

To Modify the Data:
===================
To modify the data being sent, edit the CommandResp_publisher.cxx
file where it says:
...
> objs/x64Darwin17clang9.0/CommandResp_publisher -d <domain_id> -s <sample_count>
> objs/x64Darwin17clang9.0/CommandResp_subscriber -d <domain_id> -s <sample_count>
