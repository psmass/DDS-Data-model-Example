...
unsigned int process_data(ExCmdRsp::DeviceStateDataReader *typed_reader)
{
    ExCmdRsp::DeviceStateSeq data_seq;     // Sequence of received data
    ...
    for (int i = 0; i < data_seq.length(); ++i) {
        ...
        if (info_seq[i].valid_data) {
            ...
            ExCmdRsp::DeviceStateTypeSupport::print_data(&data_seq[i]);
            samples_read++;
...
int run_subscriber_application(unsigned int domain_id, unsigned int sample_count)
{
    ...
    // This DataReader reads data on "Example ExCmdRsp_DeviceState" Topic
    DataReader *untyped_reader = subscriber->create_datareader(
        topic,
    ...
    ExCmdRsp::DeviceStateDataReader *typed_reader =
    ExCmdRsp::DeviceStateDataReader::narrow(untyped_reader);
